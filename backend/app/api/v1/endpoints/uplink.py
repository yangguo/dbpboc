        ...
        if request.selected_ids is None or len(request.selected_ids) == 0:
            ...
            if uids:
                ...
                processing_log.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] 检查已存在uid完成，"
                    f"找到 {len(existing_uids)} 个已存在，耗时: {check_time:.2f}秒"
                )
            ...
            processing_log.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] 构建插入文档完成，"
                f"准备插入 {len(to_insert)} 条记录，耗时: {build_time:.2f}秒"
            )
            ...
            if to_insert:
                ...
                processing_log.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] 数据插入完成，"
                    f"成功插入 {inserted} 条记录，耗时: {insert_time:.2f}秒"
                )
            ...
            processing_log.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] 过滤选定记录完成，"
                f"找到 {len(selected_df)} 条匹配记录，耗时: {filter_time:.2f}秒"
            )
            ...
            processing_log.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] 检查选定uid完成，"
                f"找到 {len(existing_uids)} 个已存在，耗时: {check_time:.2f}秒"
            )
            ...
            processing_log.append(
                f"[{datetime.now().strftime('%H:%M:%S')}] 构建选定插入文档完成，"
                f"准备插入 {len(to_insert)} 条记录，耗时: {build_time:.2f}秒"
            )
            ...
            if to_insert:
                ...
                processing_log.append(
                    f"[{datetime.now().strftime('%H:%M:%S')}] 选定数据插入完成，"
                    f"成功插入 {inserted} 条记录，耗时: {insert_time:.2f}秒"
                )
...
async def uplink_pending(
    ...
    """Return pending records with pagination support."""
    try:
        await _ensure_db()
        database = await get_database()
        col = database["pbocdtl"]
        ...
        if search and search.strip():
            ...
            # 在主要字段中搜索
            search_mask = (
                pending_df.get("企业名称", pd.Series(dtype=str))
                .fillna("").str.lower().str.contains(search_term, na=False) |
                pending_df.get("处罚决定书文号", pd.Series(dtype=str))
                .fillna("").str.lower().str.contains(search_term, na=False) |
                pending_df.get("违法行为类型", pd.Series(dtype=str))
                .fillna("").str.lower().str.contains(search_term, na=False) |
                pending_df.get("区域", pd.Series(dtype=str))
                .fillna("").str.lower().str.contains(search_term, na=False) |
                pending_df.get("uid", pd.Series(dtype=str))
                .fillna("").str.lower().str.contains(search_term, na=False)
            )
            pending_df = pending_df[search_mask]
        ...
        # 获取当前页数据
        page_df = pending_df.iloc[start_idx:end_idx]
...
async def uplink_export_pending(search: str = None):
    """Export all pending records as CSV with optional search filter."""
    try:
        await _ensure_db()
        database = await get_database()
        col = database["pbocdtl"]
        ...
        # 根据是否有搜索条件调整文件名
        search_suffix = f"_search_{search.replace(' ', '_')}" if search and search.strip() else ""
        filename = f"pending_data{search_suffix}_{datetime.now().strftime('%Y%m%d%H%M%S')}.csv"
